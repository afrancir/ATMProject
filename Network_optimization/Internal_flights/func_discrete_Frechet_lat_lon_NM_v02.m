function [cm, cSq, cD] = func_discrete_Frechet_lat_lon_NM_v02(P,Q) %,dfcn)
...
if nargout==3
    ...
    cD = [];
    for ind = 1:size(cSq,1)
      i = cSq(ind,1);
      j = cSq(ind,2);
      d = pos2dist_v02b(P(i,:), Q(j,:));
      cD = [cD; d];
    end
end
...
function CAij = c(i,j)
    ...
    if CA(i,j)>-1
        ...
        CAij = CA(i,j);
    elseif i==1 && j==1
        CA(i,j) = pos2dist_v02b(P(1,:),Q(1,:));     % update the CA permanent
        CAij = CA(i,j);                    % set the current relevant value
    elseif i>1 && j==1
        CA(i,j) = max( c(i-1,1), pos2dist_v02b(P(i,:),Q(1,:)) );
        CAij = CA(i,j);
    elseif i==1 && j>1
        CA(i,j) = max( c(1,j-1), pos2dist_v02b(P(1,:),Q(j,:)) );
        CAij = CA(i,j);
    elseif i>1 && j>1
        CA(i,j) = max( min([c(i-1,j), c(i-1,j-1), c(i,j-1)]),...
            pos2dist_v02b(P(i,:),Q(j,:)) );
        CAij = CA(i,j);
    else
        CA(i,j) = inf;
    end
