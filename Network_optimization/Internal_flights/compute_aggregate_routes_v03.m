...
for i=1:numel(airport_ID)
    ...
    for j=1:numel(other_airports)
        ...
        if exist(strcat(pwd,'/NETWORK/INT_Clusters/',num2str(airport_ID(i)),...
                '/',num2str(airport_ID(i)),'_',num2str(other_airports(j)),'.txt'), 'file')
            % Loading Cluster matrix
            CLUSTERS = load(strcat(pwd,'/NETWORK/INT_Clusters/',num2str(airport_ID(i)),...
            ...
            ALL_TRAJ = load(strcat(pwd,'/NETWORK/INT_traj_filt_info/',num2str(airport_ID(i)),...
            ...
            for ii=1:length(AGG_ROUTES)
                agg_route = AGG_ROUTES{ii,1};
                AGG_ROUTES_block = vertcat(AGG_ROUTES_block,agg_route);
            end
            
            dlmwrite(strcat(pwd,'/NETWORK/INT_AggRoutes/',num2str(airport_ID(i)),...
                '/',num2str(airport_ID(i)),'_',num2str(other_airports(j)),'.txt'),AGG_ROUTES_block,' ');
            ...
            xlabel('Latitude [deg]','Fontname','Avantgarde','Fontsize',14)
            ylabel('Longitude [deg]','Fontname','Avantgarde','Fontsize',14)
            grid on
            axis equal
