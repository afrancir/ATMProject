% Written by Alessandro Bombelli, Aug 1st 2016
% Routine that performs the following three operations on a pair of flight
% tracks
% 1) heading filtering: to store only the lat/lon pairs that define
% significant changes in the heading angle
% 2) Frechet distance: computation of the Frechet distance characterizing
% two flight tracks
% 3) Overlapping regions: given the Frechet distance and a user-defined
% percentage alpha (0<alpha<1), determine if and which subb-sequences of
% the two flight tracks are characterized by 
...
Q = A.ACID_info.(names{992}).lat_lon;
...
figure()
hold on
h1 = plot(P(:,2),P(:,1),'Color','b','Linewidth',2,'Marker','s','Markersize',4);
h2 = plot(P(idx,2),P(idx,1),'Color','r','Linewidth',2,'Linestyle','none','Marker','s','Markersize',4);
xlabel('Longitude [deg]','Fontname','Avantgarde','Fontsize',14)
ylabel('Latitude [deg]','Fontname','Avantgarde','Fontsize',14)
l1 = legend([h1,h2],'Flight track','Reduced flight track');
set(l1,'Fontname','Avantgarde','Fontsize',14)
axis equal
grid on

[FD,no_sol,sol] = FD_computation_v02(P_red,Q_red,1,0.05,1);

figure()
hold on
h1 = plot(P(:,2),P(:,1),'Color','b','Linewidth',2,'Marker','s','Markersize',4);
h2 = plot(Q(:,2),Q(:,1),'Color','k','Linewidth',2,'Marker','o','Markersize',4);
h3 = plot(P_red(:,2),P_red(:,1),'Color','r','Linewidth',2,'Linestyle','none','Marker','s','Markersize',6);
h4 = plot(Q_red(:,2),Q_red(:,1),'Color','r','Linewidth',2,'Linestyle','none','Marker','o','Markersize',6);
xlabel('Longitude [deg]','Fontname','Avantgarde','Fontsize',14)
ylabel('Latitude [deg]','Fontname','Avantgarde','Fontsize',14)
l1 = legend([h1,h2,h3,h4],'Flight track P','Flight track Q','Reduced flight track P','Reduced flight track Q');
set(l1,'Fontname','Avantgarde','Fontsize',14)
grid on

figure()
hold on
h1 = plot(P(:,2),P(:,1),'Color','b','Linewidth',1.5,'Marker','*','Markersize',8);
h2 = plot(Q(:,2),Q(:,1),'Color','k','Linewidth',1.5,'Marker','*','Markersize',8);
xlabel('Longitude [deg]','Fontname','Avantgarde','Fontsize',14)
ylabel('Latitude [deg]','Fontname','Avantgarde','Fontsize',14)
l1 = legend([h1,h2],'Flight track P','Flight track Q');
set(l1,'Fontname','Avantgarde','Fontsize',14)
grid on
axis equal
...
%%% active ndoes, we mean all the nodes that characterized feasible   %%%
...
if valid_overlap_sequences_ID
    ...
    [Overlap_points,P_length,Q_length] = compute_sp_ep(Merged_Sequences,P_red,Q_red,alpha,FD,Bmin,Bmax,Lmin,Lmax);
...
if is_there_overlapping
    ...
    for i=1:numel(Q_red(:,1))-1
        [lat,lon] = gcwaypts(Q_red(i,1),Q_red(i,2),Q_red(i+1,1),Q_red(i+1,2),N);
    ...
    plot(Q_red(:,2),Q_red(:,1),'Color','k','Linewidth',2,'Linestyle','none','Marker','*','Markersize',6)
    plot(Overlap_points(:,4),Overlap_points(:,3),'Color','g','Linewidth',2.5,'Linestyle','none','Marker','x','Markersize',10)
    plot(Overlap_points(:,8),Overlap_points(:,7),'Color','g','Linewidth',2.5,'Linestyle','none','Marker','x','Markersize',10)
    plot(Overlap_points(:,12),Overlap_points(:,11),'Color','r','Linewidth',2.5,'Linestyle','none','Marker','x','Markersize',10)
    plot(Overlap_points(:,16),Overlap_points(:,15),'Color','r','Linewidth',2.5,'Linestyle','none','Marker','x','Markersize',10)
    xlabel('Longitude [deg]','Fontname','Avantgarde','Fontsize',14)
    ylabel('Latitude [deg]','Fontname','Avantgarde','Fontsize',14)
    l1 = legend([h1,h2],'Flight track P','Flight track Q');
    set(l1,'Fontname','Avantgarde','Fontsize',14)
