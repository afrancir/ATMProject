function dist = pos2dist_v02b(point1, point2) %, method)
...
% Output dist is in nautical miles.
...
lat_lon_dist = sqrt( (point1 - point2)*(point1 - point2)' );
if lat_lon_dist == 0
  dist = 0;
  return
end
...
if method == 1
    ...
    R_aver = 6374*(1 + 6.8629e-04);
    deg2rad = pi/180;
    lat1 = lat1 * deg2rad;
    lon1 = lon1 * deg2rad;
    lat2 = lat2 * deg2rad;
    lon2 = lon2 * deg2rad;
    dist = R_aver * acos(cos(lat1)*cos(lat2)*cos(lon1-lon2) + ...
                         sin(lat1)*sin(lat2));
end
